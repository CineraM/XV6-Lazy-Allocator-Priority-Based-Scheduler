...
int partition(int low, int high)
{
    ...
    int j, k;
    // int pivot_bool;
    for(k=0; k<STRING_SIZE; k++) pivot[k] = eval[high][k];
    // pivot_bool = eval_bool[high];
    ...
    for(j = low; j < high; j++)
    {
        ...
        int compare = strcmp(eval[j], pivot);
        if(compare == 0)
        {
            if(eval_bool[j] == 1 && eval_bool[high] != 1)
                flag=1;
        }
        if(compare < 0 || flag)
        {
    ...
    i++;
    for(k=0; k<STRING_SIZE; k++) temp_str[k] = eval[i][k];
    for(k=0; k<STRING_SIZE; k++) eval[i][k] = eval[high][k];
    for(k=0; k<STRING_SIZE; k++) eval[high][k] = temp_str[k];

    for(k=0; k<STRING_SIZE; k++) temp_str[k] = file_contents[i][k];
    for(k=0; k<STRING_SIZE; k++) file_contents[i][k] = file_contents[high][k];
    for(k=0; k<STRING_SIZE; k++) file_contents[high][k] = temp_str[k];

    int temp_bool = eval_bool[i];
    eval_bool[i] = eval_bool[high];
    eval_bool[high] = temp_bool;
